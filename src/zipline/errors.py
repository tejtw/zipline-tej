...
class TreasuryAssetNotAvailableTooEarly(NoTradeDataAvailableTooEarly):
    '''
    Reference
    ----------
    BenchmarkAssetNotAvailableTooEarly()
    
    20230210 (by MRC) for treasury return
    '''
    pass


class TreasuryAssetNotAvailableTooLate(NoTradeDataAvailableTooLate):
    '''
    Reference
    ----------
...
class InvalidTreasuryAsset(ZiplineError):
    '''
    Reference
    ----------
    BenchmarkAssetNotAvailableTooLate()
    ...
    msg = """
{sid} cannot be used as the treasury because it has a stock \
dividend on {dt}.  Choose another asset to use as the treasury.
""".strip()
...
class IllegalDTypeException(ZiplineError):
    '''
    供以下方法使用
    ----------------------------------
    1. get_data_for_alphalens()
    2. Treasury_Return_TW()
    
    20230202 (by MRC)
    '''
    msg = ('Parameter {parameter} did not match expected type, use {dtype} instead.')

class IllegalColumnException(ZiplineError):
    '''
    供以下方法使用:
    ----------------------------------
    1. get_data_for_alphalens()
    ...
    20230202 (by MRC)
    '''
    msg = ('Column "{column}" did not exist.')
    
class IllegalValueException(ZiplineError):
    '''
    供以下方法使用
    ----------------------------------
    1. Treasury_Return_TW()
    ...
    msg = ('You choose a invalid {parameter}, try the following options instead:' + '{value}')

class EmptyOutputException(ZiplineError):
    '''
    供以下方法使用
    ----------------------------------
    1. Treasury_Return_TW()
    ...
    msg = ('function {function} return an empty {dtype}.') 
