...
from exchange_calendars.calendar_helpers import next_divider_idx, previous_divider_idx # 20230714 HRK
...
class EquityCalendarDomain(Domain):
    ...
    def all_sessions(self):
        # self.calendar -> get_calendar('calendar_name'): ExchangeCalendarDispatcher
        # ExchangeCalendarDispatcher.get_calendar -> ExchangeCalendar
        
        return self.calendar.all_sessions
    
    def all_sessions_values(self):
        self.session_values =  self.all_sessions().values.astype(np.int64)

    
    def next_open(self, date): # 20230714 created by HRK
        #
        if self.session_values is None:
            self.all_sessions_values()
        date = pd.Timestamp(date)
        idx = next_divider_idx(self.session_values, date.value)
        return pd.Timestamp(self.session_values[idx], tz=UTC)
    
    def prev_open(self, date): # 20230714 created by HRK
        if self.session_values is None:
            self.all_sessions_values()
        date = pd.Timestamp(date)
        idx = previous_divider_idx(self.session_values, date.value)
        return pd.Timestamp(self.session_values[idx], tz=UTC)
...
TW_EQUITIES = EquityCalendarDomain(CountryCode.TAIWAN, "TEJ_XTAI")     # 20230520 (by MRC) calendar由XTAI->TEJ_XTAI
# TW_EQUITIES = EquityCalendarDomain(CountryCode.TAIWAN, "XTAI")       # 20230520 (by MRC) calendar由XTAI->TEJ_XTAI
