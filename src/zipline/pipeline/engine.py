...
class SimplePipelineEngine(PipelineEngine):
    ...
    def _compute_root_mask(self, domain, start_date, end_date, extra_rows):
        ...
        if start_date not in sessions:
            # 20230714 fixed by HRK
            # Shift non-trading start date to the next trading date. 
            start_date = domain.next_open(start_date)
            if start_date not in sessions:
                raise ValueError(
                    "Pipeline next start date ({}) is not a trading session for "
                    "domain {}.".format(start_date, domain)
                )

        elif end_date not in sessions:
            # 20230714 fixed by HRK
            # Shift non-trading end date to the previous trading date.
            end_date = domain.prev_open(end_date)
            if end_date not in sessions:
                raise ValueError(
                    "Pipeline previous end date {} is not a trading session for "
