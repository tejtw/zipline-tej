...
class DataFrameLoader(implements(PipelineLoader)):
    ...
    baseline : pandas.DataFrame
        ...
        Index.  Dates should be labelled with the first date on which a
       ...
       （DataFrameLoader的load_adjusted_array原先不會往前平移一期，day N最近已知的資料就是day N當天資料，
        為了一致性這邊也修改為往前平移一期。所以往後OHLCV使用這個class時不用再往前shifted）   # 20230729 (by MRC)
    ...
    def __init__(self, column, baseline, adjustments=None):
        ...
        # (by MRC) 以下修正pipeline的文字類型欄位有None以外的缺值會出現ValueError的問題
        #self.baseline = baseline.values.astype(self.column.dtype)
        base = baseline.values.astype(self.column.dtype)
        if self.column.dtype == 'O':
            base[isnull(base)] = str('') #None 'N/A'
        self.baseline = base
